...
**Goal completion gate (per the glossary):** every `[consolidated]` term in `.claude/context/INDEX.md` must carry all four `[sync:X]` tags AND `[verified]`, across three consecutive verifier-clean rounds with zero new drift surfaced.
