...
Here is the move that makes the multiplier honest: the multiplier is the *worth of each action* toward completing the phase. A deep phase needs many actions, so you pick a small multiplier — each action counts for less, and the gate fills through real volume. A surgical phase needs few actions, so you pick a large multiplier — each action counts for more, and the gate fills fast. The dial is not declaring a workload; it is declaring how much each move is worth against a fixed hundred-point gate. *[ref: voice-multiplier-action-worth | .claude/plugins/phase_observe/hooks/voice.xml entry-set-multiplier voice + multiplier-zero-block | The OBSERVE entry voice frames the multiplier as "the worth of each action toward completing the phase — a deep phase needs many actions: pick small so each counts for less and the phase fills through real volume; a surgical phase needs few: pick large. The gate is a floor, not a target." Action-worth-floor model per CONTEXT.opevc "Action-worth floor." Both voices carry the action-worth-floor text: the `hooks/voice.xml` entry voice frames the dial as the worth of each action toward the 100-point floor, and the `scripts/voice.xml` info voice `observe.info.multiplier-set` confirms the same on unlock — "the worth of each action toward the phase's 100-point floor — small for a deep phase (many actions), large for a surgical one (few actions). The floor is not a target."]*

And the hundred-point gate is a *floor*, not a target. Cross it and the phase is allowed to advance — but the agent is meant to keep working past it whenever the work still needs more. The upper side of the gate is deliberately unguarded. There is no penalty for over-shooting, no waste in collecting more points than the floor requires, because points are never the thing the agent is chasing. The floor only marks the minimum honest depth for a phase of the declared size; the work itself decides when the phase is actually done.

That is why the forecast carries no symmetric risk to balance. Pick a multiplier too large for the work and the floor sits high — the agent simply keeps doing the phase's real work until it crosses, because the work was always the point. Pick one too small and the floor opens early, but a later phase (verify especially) catches any gap and forces a backward transition; the under-served phase re-enters and finishes what should have been done. The dial is not a bet with a downside on each side. It is a forecast that sets the honest minimum, and the work runs past it as far as the work demands.

The anti-rush pressure never comes from the number. It comes from the coaching that speaks the language of the work — observe more, hunt the blindspots, analyze deeper, change your lens, double-check the claim. The agent never hears "you are short on points." It hears what kind of work the phase still wants. *[ref: coaching-speaks-work-language-not-points | .claude/plugins/phase_observe/hooks/voice.xml tier-distance coaching voices + observe-tracker.sh dispatch | The tracker dispatches coaching by distance from the gate, but the tier-distance coaching voices speak in work-language (read before writing, synthesize what you have gathered, hunt the blindspot) rather than naming a point count. The agent is steered by what the phase needs, not by a visible score — consistent with the invisible-points principle.]*

The point of the dial, then, is not to find a safe choice on a two-sided risk — there is no two-sided risk. The point is to force a real act of meta-cognition before the phase begins: to put an honest number on the felt size of the phase, set the floor, and then do the work all the way through.
...
The inverse-multiplier pattern — an inverse-direction scalar set at entry, forecast before the work begins, scoring an invisible point system against a hidden floor — lifts off the prototype into any work where forecasting scope up front sharpens the work. A consulting practice cultivating an engagement-delivery seed could install a per-engagement scope dial — *surgical*, *standard*, *deep* — that forces the partner to forecast scope before the billable work begins, with the agent's actions silently scored against a hidden floor the partner never sees. The floor sets the honest minimum depth for an engagement of the declared size; the work runs past it as the brief demands. Forecast a touch small and the floor opens early, but the deliverable's own checks reopen the gap before the engagement closes — the forecast sets the floor, and the work, not the dial, decides when the engagement is done.
...
The deepest payoff of the multiplier is the cognitive failure mode it prevents: the *unforced phase*. An agent that enters a phase without a felt size for it falls back on whatever the previous phase looked like, or whatever the last cycle's phase looked like, or whatever the prompt seemed to want. None of those is the *current phase's* honest scope. The dial forces the agent to put a number on the felt size before the tools unlock. The number is sometimes a little off; that is fine. The floor it sets is a minimum, and the work runs past it as far as the work demands. The friction is the pedagogy. The dial is the compartment.
