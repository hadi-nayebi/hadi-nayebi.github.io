...
There is one more kindness in that fresh entry. When the agent backs into completed work to revise it, the exit gate does not make the correction re-pay the full reflection wall. A backward re-entry clears with only the constitutive CORE floor — the nuanced-reflection minimum and the reflector subagent are waived, because the heavy reflection was already done on the forward pass. The wall is a forward-pass cost. A backward visit is a targeted fix, not a fresh investigation, so it stays cheap; a phase the agent keeps returning to never traps it in an endless re-payment of work it already finished. *[ref: forward-backward-gate-asymmetry | .claude/context/opevc-metacog.md "Three-family exit gate" (Forward/backward gate asymmetry) | The full three-family wall is a FORWARD-pass cost; a BACKWARD re-entry (entry_from is not the phase's forward origin) clears with only the constitutive CORE floor — exempt from family-a's nuanced minimum and family-c's reflector. Each `<phase>-commit.sh` reads the entry's `entry_from` and branches; forward origins are OBSERVE←idle, PLAN←observe, EXECUTE←plan, VERIFY←execute|plan, CONDENSE←verify; any other value is backward. Built 2026-06-24.]*
