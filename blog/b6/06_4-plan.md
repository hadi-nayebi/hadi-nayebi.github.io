...
`yaml_drafting` lives on a SEPARATE dependent yaml-job, not on this md-job. After `md_approved` lands, this md-job's CONDENSE spawns a dep yaml-job via a `[PENDING-JOB]` marker (same basename, .yaml extension). When activated, the dep job's cycle 1 PLAN calls `set-plan-file plan_<name>.yaml` and the extension parser lands `plan_state = yaml_drafting` directly. EXECUTE writes the .yaml; VERIFY refines across however many cycles the dep job runs, with backward to PLAN allowed when schema-level analysis needs revisiting. `yaml_ready` is the second user-gated flip on the dep job, and from that point the .yaml is injected at phase entry on every cycle the dep job runs — the plan is now actively shaping the agent's context. *[ref: dep-yaml-job-state-arc-and-injection-at-phase-entry | .claude/plugins/phase_plan/scripts/plan.sh:386-392 yaml extension parser + L697-701 dep-yaml-job NOTE + .claude/plugins/phasic_system/hooks/phase-init.sh write_yaml_cache | The extension parser sets plan_state directly to yaml_drafting when set-plan-file is called with a .yaml name on a fresh dep job (plan_file=null). The L697-701 NOTE confirms: "md-jobs end at md_approved and never flip to yaml_drafting." approve-yaml requires plan_state==yaml_drafting (on the dep job), then sets plan_state=yaml_ready. From that flip on, phase-init.sh's write_yaml_cache runs at every phase transition of the dep job: reads phase.sh --hook read-yaml for the current cycle+phase, writes the JSON field-map to the yaml-injection cache that voice-helper augments every matching voice with (appending by default; mode-aware replace/prepend per yaml entry — see B6.10 for the three modes).]*
...
The `.md` and the `.yaml` live side-by-side once both exist (same basename in `.claude/knowledge/plans/`), and they are not the same artifact wearing different masks. The `.md` belongs to the parent md-job; the `.yaml` belongs to the dependent yaml-job spawned at md-job CONDENSE. The `.md` is the human-readable accumulating prose; the `.yaml` is the parseable injection target the orchestrator reads at phase entry to pour context-specific content into the working session. *[ref: yaml-injection-cache-populated-at-phase-entry | .claude/plugins/phasic_system/hooks/phase-init.sh write_yaml_cache function | At every phase transition, write_yaml_cache reads the .yaml's current cycle+phase field-map via phase.sh --hook read-yaml and writes it as JSON to YAML_INJECTION_CACHE. voice-helper.sh consumes the cache jq-lookup-per-voice: any voice id matching a yaml key gets its value augmented into the rendered text at fire time — appended (the back-compat default), replaced, or prepended depending on the yaml entry's mode. The yaml is per-cycle-per-phase context dripped into the agent's working voice stream.]*
