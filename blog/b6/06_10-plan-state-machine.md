...
<!-- RAW_HTML -->
<figure class="blog-image" data-visual-style="I90-A10" data-information-weight="90" data-artistic-weight="10" data-visual-role="storytelling" data-visual-batch="5" style="margin: 2rem 0;">
  <img src="images/plan-file-shape-counting-b6-10-v2.jpg" alt="Chalk diagram showing the cycle-one PLAN decision among Stage 1 without a plan file, Stage 2 with a Markdown plan, and Stage 3 with a YAML plan, followed by the cycle-count completion rule in CONDENSE." style="display: block; width: 100%; max-width: 100%; height: auto; margin: 0 auto; border-radius: 8px;">
  <figcaption>The plan file selects a shape; completion follows a counting rule, not a named-stage state machine.</figcaption>
</figure>
<!-- /RAW_HTML -->
...
The plan file owns the long-horizon memory of a job. One field carries it — `plan_file`, living on the job's `phase_plan` extension (keyed to the job by its ID, stored in `phase_plan`'s own `data.json`): the literal `false` when the job has no plan, or the name of a working document on disk (`<name>.md` or `<name>.yaml`) when it does. The document is the memory; the field is the pointer. *[ref: plan-file-field-three-values | private historical prototype | Claim checked against a private historical prototype.]*
...
The **compaction file** from [the always-on cortex](../b5/05_3-brain-guard.html) is **backward and reflective** — it carries cross-*session* cognitive state: how the work went, the loops the agent got stuck in, the blind spots it hit, and the handful of top-level facts that must survive the next context clear. *[ref: compaction-vs-plan-boundary | private historical prototype | Claim checked against a private historical prototype.]*
