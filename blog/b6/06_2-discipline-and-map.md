...
The footer markers from [the previous essay series](../b5/05_1-the-two-layer-foundation.html) — `---Ob---`, `---Pl---`, `---Ex---`, `---Ve---` — are physical barriers. Each phase's guard enforces one rule: writes must land strictly below that phase's own marker. The constraint is asymmetric — each phase can write only into its own footer block and the blocks that follow it in the cycle, never the blocks already closed upstream. As the cycle progresses, the editable region shrinks from above: OBSERVE writes anywhere below its marker into all four blocks, PLAN into three, EXECUTE into two, VERIFY into just the last. Each completed phase becomes part of the locked upstream record. The body above all four markers is reserved for CONDENSE; none of the four work-on-project phases can touch it. *[ref: per-phase-marker-floor-enforced | .claude/plugins/lib/section_guard/section-check.sh check_section_edit + .claude/plugins/phase_observe/hooks/observe-guard.sh + .claude/plugins/phase_plan/hooks/plan-guard.sh + .claude/plugins/phase_execute/hooks/execute-guard.sh + .claude/plugins/phase_verify/hooks/verify-guard.sh | The shared check_section_edit helper takes a phase_marker plus file path plus old/new strings and rejects any edit that crosses an upstream phase-section boundary inside any CLAUDE.md. Each of the four phase guards invokes the helper with its own marker (---Ob---, ---Pl---, ---Ex---, ---Ve---), producing the asymmetric write authority: OBSERVE may write into all four blocks below ---Ob---, PLAN into three below ---Pl---, EXECUTE into two below ---Ex---, VERIFY into the single block below ---Ve---. The body above ---Ob--- is forbidden to all four — reserved for CONDENSE.]*
...
The result: each phase produces its own kind of artifact. OBSERVE produces working memory. PLAN produces a plan document. EXECUTE produces code changes plus execution notes. VERIFY produces pass/fail results. CONDENSE produces a clean working memory and durable knowledge files. *[ref: per-phase-artifact-kinds | .claude/plugins/CLAUDE.md "Active Plugins" section | The Active Plugins table declares each phase plugin's objective verbatim — phase_observe: "Context gathering into CLAUDE.md with point system"; phase_plan: "Transform observations into actionable steps. Strict read whitelist."; phase_execute: "Build phase with altered-list scope enforcement, multi-commit, point system."; phase_verify: "Cycle judge + teacher. Scripts, web verification, 3 backward transitions."; phase_condense: "Knowledge extraction, 80% deflation gate on bottom-section words, lock-forward to idle." Per-phase artifact kinds are encoded in those role declarations.]*
...
The agent commits as often as it wants inside a phase. Most of those are ordinary saves — a plain `git commit` to checkpoint a working draft, split a messy change into reviewable chunks, or bank a recovery point before a risky edit. These are **intermediate commits**, and they pass through untouched: no gate fires, no count is checked, nothing blocks. They are bookkeeping, and bookkeeping should be cheap. *[ref: intermediate-commits-are-ungated | CONTEXT.md "Intermediate vs force-advance commits" section | The canonical glossary defines two commit kinds: "Intermediate commits — plain `git commit -m \"...\"` invocations during phase work. NO gate enforcement. Useful for checkpointing partway-through work, splitting changes into reviewable chunks, or recovering from a botched edit. Don't fire shape, points, or custom gates." The EXECUTE essay's checkpoint pattern is exactly this kind of commit.]*

Exactly one commit per phase is different: the one that *advances* the phase. To cross a boundary — OBSERVE handing off to PLAN, EXECUTE handing off to VERIFY — the agent runs the phase's force-advance commit (`<phase>-commit.sh --force`). That commit is the only one the orchestrator gates. It checks the phase's point count (the hundred-point threshold from [Essay 6.8](06_8-inverse-multiplier.html)), it checks the phase's custom requirements, and only if all of them clear does the cycle move forward. The design intent is plain: spend the agent's care where the decision actually matters — at the boundary where one mode of thinking hands its work to the next — not on every routine save in between. *[ref: force-advance-is-the-gated-boundary-commit | CONTEXT.md "Intermediate vs force-advance commits" + "Per-phase points" sections | The glossary: "`--force` advance commits — `<phase>-commit.sh --force`. THE phase-boundary commit. Gated by Phase commit shape + 100 Per-phase points + any per-phase custom gates … Successful `--force` advances the phase." And the design intent verbatim: "concentrate the cognitive load on phase-boundary decisions. The agent can commit freely during phase work without tripping gates; the gate fires ONCE per phase, at the boundary." The 100-point threshold is the per-phase forward-advance gate.]*

### The shape each boundary will demand

There is a third discipline the design layers onto that boundary commit, and it is worth naming now even though it is not yet wired into the prototype's commit scripts — the same honesty [Essay 6.6](06_6-verify.html) keeps about VERIFY's own gate. It is the sibling of the question shape from [the always-on series](../b5/05_6-question-discipline.html): a **commit shape**.

The idea is that each phase's force-advance commit would have to carry a required structure of named `## ` sections in its body, checked before the boundary opens — so the commit is not free prose but a small filled-in form. The form differs by phase, because each phase produces a different kind of evidence. OBSERVE would record what it observed, the sources it consulted, what it resolved, and what is still open. PLAN would record the plan for this cycle, the acceptance criteria, the risks, and how VERIFY will test it. EXECUTE would record what it implemented, the evidence, and which plan item each change addressed. VERIFY would record the criteria it checked, the pass-or-fail verdict on each, the evidence, and any outstanding items. CONDENSE would record the waterfall steps it completed, the knowledge it routed, and the markers it consumed. *[ref: phase-commit-shape-per-phase-catalog | CONTEXT.md "Phase commit shape" section | The canonical design names the per-phase catalog verbatim: observe-commit → "What I Observed / Sources Consulted / Questions Resolved / Questions Remaining"; plan-commit → "Plan This Cycle / Acceptance Criteria / Risks / How VERIFY Will Test"; execute-commit → "What I Implemented / Evidence (commit hashes) / Plan Item Addressed"; verify-commit → "Acceptance Criteria Checked / Pass-Fail Per Criterion / Evidence / Outstanding Items"; condense-commit → "Waterfall Steps Completed / Knowledge Routed / Markers Consumed." The glossary marks this enforcement as design-canonical, slated for each phase's commit script per the Addition ledger — not yet built.]*

On top of that shape, each phase's boundary would also enforce its own one-off check — the custom gate named per phase in the operational map just below. OBSERVE's first cycle would have to expand the job's objective before it can advance; PLAN's first cycle would have to record its plan-file decision; EXECUTE's first cycle of a multi-cycle job would have to confirm the plan file actually exists; VERIFY would have to show an empty outstanding-items list and a plan file it did not quietly edit; CONDENSE would have to hit its deflation target and clear its markers. *[ref: per-phase-custom-gates-layer-on-shape | CONTEXT.md "Phase commit shape" section (per-phase custom gates table) | The design's custom-gate catalog: OBSERVE cycle 1 → Objective expansion + Extended shape sections; PLAN cycle 1 → `plan_file` decision recorded; EXECUTE Stage 2/3 cycle 1 → plan file exists at declared path; VERIFY → `## Outstanding Items` empty-content gate AND plan-file-not-edited gate (else backward-advance); CONDENSE → 80% deflation + `[JOB-COMPLETE]` eligibility + markers consumed. These layer ON TOP of the universal shape + 100 points. All design-canonical per the Addition ledger; not yet enforced by the commit scripts.]*

The point of all this is cumulative. Each boundary deposits a small, machine-readable record of what the phase actually did. By the time a long job reaches its final cycle, that trail already says, commit by commit, that the work was observed, planned, built, and verified — so the closing `[JOB-COMPLETE]` question is a thin reference to evidence that already exists, not the one place the whole job's quality is decided. The shape is design-canonical and the catalog is fixed; what is still pending is the wiring that would make each phase's commit script enforce it. *[ref: commit-shape-incremental-evidence-trail | CONTEXT.md "Phase commit shape" section (incremental-discipline paragraph + double-verify system) | The glossary: "The discipline is INCREMENTAL: each phase deposits machine-parseable evidence of what it did. By the time the focused job reaches the final cycle's CONDENSE, the evidence trail across prior commits is definitive — `[JOB-COMPLETE]` becomes a thin final reference to existing evidence, not the only quality gate." The double-verify system runs the `## Outstanding Items` empty-content check at both cycle granularity (VERIFY) and job granularity (CONDENSE). Enforcement is slated per the Addition ledger ("Add Phase commit shape enforcement to each phase's commit script per the catalog"), not yet wired.]*
...
**OBSERVE** — gather context before any plan can form. *[ref: observe-cycle-1-expands-objective | CONTEXT.md "Objective expansion" section + "Job Stage" section | Objective expansion is OBSERVE's cycle-1 custom force-advance gate: it grows the job's initial objective into a fuller statement as the agent loads context. The Stage itself is decided in cycle-1 PLAN (the set-plan-file call or non-call), NOT in OBSERVE — OBSERVE feeds the decision with context; PLAN makes it.]*
- On cycle 1, expand the job's objective from its short creation stub into a full statement of intent — OBSERVE's distinctive cycle-1 work; the Stage decision (whether PLAN names a plan_file) is made next, in PLAN
...
**VERIFY** — judge prior work with independent eyes. *[ref: verify-refines-plan-forces-backward | .claude/plugins/phase_verify/hooks/verify-guard.sh:262-286 plan-edit + :360-374 job-graph allowlist + CONTEXT.md "Plan-verify backward loop" section | The verify-guard's Edit|Write case-arm allows writes to the focused job's declared plan_file (.md or .yaml) and blocks every other /knowledge/ path with `verify.guard.knowledge`. Editing the plan file blocks forward-advance — the agent commits --backward plan and PLAN re-cognizes; VERIFY has no approval or seal step (completion is CONDENSE's). The Bash case-arm whitelists job.sh remove-dependency (VERIFY's lifecycle-symmetry partner to CONDENSE's add-dependency) and blocks create/create-dependent/add-dependency, steering those to a [PENDING-JOB] marker for CONDENSE step 3.]*
...
**CONDENSE** — consolidate the cycle's learnings into the brain. *[ref: condense-7-step-waterfall-owns-job-creation | .claude/plugins/phase_condense/hooks/condense-guard.sh:311-331 | The condense-guard's Bash case-arm whitelists job.sh add-dependency alongside show/focused/list/complete/update (the condense-specific lifecycle operations), and allows job.sh create / create-dependent directly — gated only by a fix-in-cycle scope check that blocks creating a job for work that should be a backward transition instead (the --out-of-scope flag waives the check for genuine next-cycle work). CONDENSE is the only phase with cycle-wide context for graph mutations — VERIFY can remove dependencies, but additions belong here.]*
