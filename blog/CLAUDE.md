...
| 5.1 | `05_1-the-two-layer-foundation` | Pt 1: The Two-Layer Foundation | **drafting v0.3.0** — ~2,441w / 9 min · 9 refs · HTML rebuilt · transcript regen'd (final:false) · audio pending. v0.3.0 iter-14.a HIGH+MED fixes: L45 (202w) split into layer-intro + tool-restrictions paragraphs; L49 (170w) split into substrate/always-on + phasic-layer-CONDENSE-waterfall paragraphs; evolution-cap path ref corrected (scripts/→hooks/). |
| 5.2 | `05_2-plugin-integrity` | Pt 2: `plugin_integrity` — Plugin Edit Safety | **drafting v0.3.0** — ~2,059w / 8 min · 7 refs · HTML rebuilt · transcript regen'd (final:false) · audio pending. v0.3.0 iter-14.a HIGH+MED fixes: evolution-cap path ref corrected (scripts/→hooks/); L33 (161w) test-gate density wall split (test-gate punch + close-triggers-suite mechanism); L67 (181w) hope-chain split from consequences/composition. |
...
| 5.4 | `05_4-job-core` | Pt 4: `job_core` — Job Lifecycle | **drafting v0.2.0** — ~1,500w / 7 min · HTML built · transcript regen'd (final:false) · audio pending. v0.2.0 iter-14.b fixes: ref-tag line range L25 corrected (50-74 → 49-74); shared-key paragraph split L33 (219w → 104/115); refusal-to-stop paragraph split L52 (201w → 91/124) + "and so on" → "every OPEVC phase" per Rule 1. |
| 5.5 | `05_5-interaction-summary` | Pt 5: `interaction_summary` — Cross-Session Memory | **drafting v0.3.0** — ~1,000w / 5 min · HTML built · transcript regen'd (final:false) · audio pending. v0.3.0 iter-14.b fixes: BYPASS_PREFIXES guard count corrected (six → five, named) at L59; enforcement paragraph split L45 (217w → 117/100); Trigger-Mechanism ref-tag L25 verified no-drift. |
| 5.6 | `05_6-question-discipline` | Pt 6: `question_discipline` — Structured Questions | **drafting v0.3.0** — ~1,200w / 5 min · 4 refs · HTML rebuilt · transcript regen'd (final:false) · audio pending. v0.3.0 iter-14.b fixes: batch-cascade paragraph split L29 (204w → 135/48 prose); shape-cognition paragraph split L33 (198w → 111/89 prose) with ref-tag preserved on enumeration half; LOW split applied L37 (182w → 148/74 prose) before "The same discipline generalizes". |
| 5.7 | `05_7-claude-md-hierarchy` | Pt 7: The CLAUDE.md Hierarchy | **drafting v0.2.0** — ~3,300w / 12 min · HTML rebuilt · transcript regen'd (final:false) · audio pending. v0.2.0 iter-14.c HIGH+MED fixes: count drift corrected (113/119 → 110/115 inside `.claude/`, scoped to substrate); `.claude/CLAUDE.md:8-47` ref-tag age-proofed (section-name pointers, not line numbers, after v0.3.0 reshape); memory-files ref-tag age-proofed (specific feedback count → "many typed entries"). |
| 5.8 | `05_8-historian-ratchet` | Pt 8: The Historian Ratchet | **drafting v0.2.0** — ~2,500w / 10 min · HTML rebuilt · transcript regen'd (final:false) · audio pending. v0.2.0 iter-14.c HIGH+MED fixes: lock-manager.sh ref-tag range widened L351-360 → L351-378 (covers comment-block + jq emit at L378); voice.xml ref-tag corrected L151 → L154; self-acknowledging "Line drift from earlier authorship" parenthetical removed (no longer needed after accurate range). Folds in v0.2.0 changes shipped earlier in commit d40661d (series-aware refs + historian count fix) whose frontmatter version was not bumped at the time. |
| 5.9 | `05_9-customization-guardrail` | Pt 9: The Customization Guardrail | **drafting v0.3.0** — ~2,400w / 10 min · 5 refs · HTML rebuilt · transcript regen'd (final:false) · audio pending. v0.3.0 iter-14.c MED fix: birth-branch line pointer corrected (around 297 → around 313). v0.2.0 tightened propose-and-confirm flow: item 1 trigger-list compressed (specifics → new ref-tag); item 4 collapsed 112w → 17w body, ref-tag absorbs Post-handler implementation detail + data.json shape fact. |
