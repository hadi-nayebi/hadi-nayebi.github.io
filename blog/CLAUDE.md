...
| 5.6 | `05_6-question-discipline` | Pt 6: `question_discipline` — Structured Questions | **drafting v0.2.0** — ~1,200w / 5 min · 4 refs · HTML built · transcript regen'd (final:false) · audio pending. v0.2.0 added shape-driven cognition section (per-prefix admission gates, body+options principle, `[WAITING]` 2-section design) + 3-tile stencil image placeholder replacing registry-cascade-rejection concept. |
...
| 5.9 | `05_9-customization-guardrail` | Pt 9: The Customization Guardrail | **drafting v0.2.0** — ~2,400w / 10 min · 5 refs · HTML rebuilt (clean) · transcript regen'd (final:false) · audio pending. v0.2.0 tightened propose-and-confirm flow: item 1 trigger-list compressed (specifics → new ref-tag); item 4 collapsed 112w → 17w body, ref-tag absorbs Post-handler implementation detail + data.json shape fact. Item 4 ref-tag rewritten to avoid `[[:space:]]` regex char-class that broke the converter (markdown leak cascade L171-191 in v0.1.0 HTML — fixed by semantic re-description). |
