...
**Ground truth:** the canonical glossary is `hadosh_academy/.claude/context/INDEX.md`. Per Rule 40, when an essay body (or this file) conflicts with a `[consolidated]` definition there, the prose is the drift — fix the prose, not the glossary. These compact glosses load for **every** blog edit so the canonical names + their banned aliases stay in working memory; full definitions + implementation pointers live in `hadosh_academy/.claude/context/INDEX.md` (+ its cluster files). Series-exclusive terms live deeper (see the foot of this section).
...
- **Dependency removal (VERIFY)** — VERIFY reviews the focused parent's OPEN (pending/active) deps each cycle and resolves each: KEEP (still needed), PROMOTE (`remove-dependency` — unlink only, the child survives as standalone work), or VOID (`void-dependency` — unlink AND mark the child `voided`). Lifecycle-symmetry: CONDENSE adds deps, VERIFY removes them. _Avoid:_ "delete the dependency," "remove-dependency deletes/unfocuses the child," CONDENSE-removes-deps.
- **voided** — the fourth job status: a job abandoned without completion, kept on disk for diagnostics (never deleted), Stop-gate-exempt (the gate counts only `active`/`pending`). Reached when VERIFY runs `void-dependency` on dead work. _Avoid:_ "voided = deleted," "voided is a waiting/blocked state," conflating `voided` (an already-born job parked terminal) with **reject** (CONDENSE declining a `[PENDING-JOB]` note — no job is ever born).
...
- **Focus lifecycle** — the temporal arc of the `focused` flag: SET by `job.sh focus` (and the prompt-hook bootstrap; focusing one job atomically auto-unfocuses the prior — single-focus invariant); KEPT through completion (a completed job stays focused for a cleanup window in its final CONDENSE); CLEARED by `job.sh pause` or by the **condense→idle advance** (the relocated unfocus, via the `clear-focus` arm — NOT by `job.sh complete`). _Avoid:_ "unfocus command" (none — unfocus is a side effect), "complete unfocuses the job" (completion keeps focus; the advance unfocuses).

**Plugin layers**
- **Always-on layer** — the set of plugins that fire on every prompt, every tool call, and every session start regardless of which job is focused or which phase it is in — currently five in the prototype: `plugin_integrity`, `job_core`, `interaction_summary`, `brain_guard`, `question_discipline` (each registers its hooks in `.claude/settings.local.json`; membership is a code fact). The contrast partner is the phasic layer, which mode-switches per phase. _Avoid:_ **"background layer"/"background plugins" (BANNED)**, "persistent layer," "system plugins," counting `phasic_system` among the always-on five (it is the phasic-layer orchestrator).
- **Phasic layer** — the other plugin layer: it activates **one phasic mode at a time**, dictated by which phase the focused job is in, while the always-on plugins keep running underneath. Membership is an **orchestrator + five phase plugins**: `phasic_system` (conducts the phase-transition state machine and per-job phase/cycle state) plus `phase_observe` / `phase_plan` / `phase_execute` / `phase_verify` / `phase_condense` (one per phase, each enforcing that phase's tool split in code). It is what makes the seed "operate differently in each compartment." _Avoid:_ "phasic_system is always-on" (by concern it is the phasic-layer orchestrator; only its firing pattern is continuous), "six phase plugins" (there are five phase plugins + one orchestrator), "the phasic layer replaces the always-on layer per phase" (always-on runs UNDERNEATH the active phasic mode, concurrently).
...
- **OPEVC cycle** — the named five-phase SEQUENCE the focused job's cognition laps: O→P→E→V→C, then back to idle. The acronym names the *sequence*; the five compartments themselves are the *Phase* term. idle is qualitatively different in kind — the minimal, non-OPEVC compartment for inter-job work (switching focus, opening/closing a lap); the cycle counter ticks +1 at each `idle→observe` entry, NOT at `condense→idle`. The whole phase-set is a customization surface (idle can gain responsibilities, new phases can be added) — OPEVC is this prototype's answer, not a law. _Avoid:_ "OPEVC phases" as a synonym (phases = the compartments, OPEVC = the sequence), "the loop" (ambiguous with intra-phase backward loops), framing phases by graph-position ("the ring," "node vs edge").
- **Execution notes** — the short, cycle-specific FREE-FORM entries EXECUTE writes below `---Ex---` alongside its code: surprises, judgment calls where the plan left a gap, what the next phase should know — the UNPLANNED record. Ticking off the PLAN-authored execution checklist (recording planned items *done*) is a separate activity under the same footer, NOT an execution note; the commit-body message is a separate surface again. They flow forward to VERIFY and into CONDENSE's footer-to-body absorption. _Avoid:_ "execution log," "commit notes," "checklist ticks."
...
- **Altered list** — the mechanism by which OBSERVE/PLAN CLAUDE.md edits scope EXECUTE's write access: each CLAUDE.md a phase touches declares its directory editable, the list is FROZEN at EXECUTE entry (copied from the observe+plan record into `altered_claude_md`), and `execute-guard` enforces it — exact-directory match, NOT recursive. The build→freeze→enforce lifecycle is what makes "scope is declared upstream, enforced downstream" real. _Avoid:_ "edit whitelist," "recursive scope," "file scope."
- **Per-phase write scope** — the asymmetric tool-access compartment each Phase guard enforces (WHAT each phase may touch — distinct from Section-anchor write authority, which governs WHERE in a CLAUDE.md a phase writes): OBSERVE (read-only project; Glob/Grep/Web free — the gathering phase); PLAN (read-only project; Glob/Grep/Web blocked — pure synthesis of what OBSERVE gathered); EXECUTE (full write inside the altered list; Glob/Grep/Web blocked — it produces, not investigates); VERIFY (read-only project; Glob/Grep allowed to find tests, Web allowed only to check findings against external reality); CONDENSE (no project files; any `.md` + `voice.xml`, no section restriction). The restriction IS the pedagogy — a phase cannot skip its work because the tools to skip it are blocked. _Avoid:_ "phase permissions," "file access control," "sandbox."
- **Section-anchor write authority** — the rule (enforced by the shared `section_guard`) governing WHERE and WHO writes a CLAUDE.md: WHERE — a phase writes only at or below its own footer anchor, never above (cascading-downward: OBSERVE writes all four footers, VERIFY writes `---Ve---` only; the body above `---Ob---` is CONDENSE-only); WHO — in O/P/E/V the seed (main session) alone writes the CLAUDE.md layer (subagents are blocked from CLAUDE.md edits; CONDENSE excepted). Produces the forward-write asymmetry: notes flow downward and forward, never reaching back up. _Avoid:_ "section lock" (it is a directional write rule, not a lock), "footer protocol" (that names the CONDENSE-side mechanic — the four-footer protocol), "above/below rule" (imprecise).
...
- **`urgency` field** — a per-job priority field governing how aggressively a NON-focused job demands attention away from the focused one (the seed's cognitive immune system — a job raises its own urgency on discovering something critical, interrupting the current focus). SPLIT ship state: v1 is IMPLICIT — no explicit field; the idle-phase next-job-selection voice coaches the seed to weigh apparent urgency by hand. The post-oss form is an EXPLICIT field with three levels (normal / semi-urgent / very-urgent). Distinct from `reactivate_target` (that decides WHERE a reactivated repeating job lands; urgency decides HOW HARD a live job competes for focus). _Avoid:_ priority, conflating with `reactivate_target`.
...
**Phasic-compaction** (BUILT + active across all five phases; taught in b5/b6)
...
- **Three-family exit gate** — THE per-phase advance condition: (a) reflection commands ran [hard], (b) new marked notes of any type added [soft], (c) ≥1 of the phase's available reflector subagents ran [hard]. Families (a)/(b) check a variable minimum drawn per phase-entry from a set (the 2 CORE ops are a constitutive baseline, not counted toward it); family (c) reads the per-entry `agent_launches` record (user-extensible reflectors, never one pinned name). CONDENSE additionally keeps its 80% deflation gate, and its marker-side check runs in the CONSUMPTION direction (per-class subagent runs ≥ marked notes), not creation. Armed unconditionally across all five phases (the `METACOG_GATE_ENABLED` master flag was deleted — user ruling 2026-06-11). _Avoid:_ "a point threshold," "replaces the 100-point total" (no point system exists to replace), "exempts CONDENSE."
- **Per-phase accrue design** — every OPEVC phase becomes an operational→metacognitive flow: the ENTRY voice introduces operational subagents (do the work), the EXIT voice fires at the advance boundary and delivers a mindset shift (stop doing the phase, examine your own cognition; here is the gap — reflections / marked notes / a reflector run — satisfy it then advance). Reflectors are dispatched DURING the phase and accrue the compaction file, NOT launched by the exit voice. _Avoid:_ "one giant metacog phase at the end," "metacog is optional," "the exit voice launches the reflector."
...
**Identity & customization (P-cluster)**
- **Customization guardrail** — the OR-gate deciding when ANY plugin-code work is admitted (editing an existing plugin OR creating a new one route through the same gate): the lock-manager admits `[PLUGIN-LOCK]` iff the phase is gmode OR the focused job's `plugin_lock_approval` is true; the free surface (CLAUDE.md, docs, voice.xml, agent-defs) is exempt. An inherited safety floor — its two-route admission is inherited intact, its knobs (prefix registry, ≥100-word gmode floor) are tunable. _Avoid:_ "permission check"/"ACL," "PLUGIN-LOCK" as a synonym (the guardrail gates the question, it is not the question), **"single-gate" (BANNED)**, "AND-gate" (admission is OR; only the block needs both arms to fail), "read-only"/"frozen."
- **Propose-and-confirm flow** — the five-step agent-noticed route through the customization guardrail: a phase notices a need → CONDENSE proposes via `[JOB-APPROVE-CREATION]` → the user judges → on approve the job is created and `plugin_lock_approval` flips → the agent focuses + activates. Enforces Identity Fact 3 (the seed cannot self-authorise plugin work). The mid-life raise (`[JOB-APPROVE-PLUGIN]`, any active phase) is the second propose-confirm route. _Avoid:_ "auto-create the job" (user-gated, never self-authorised), "any phase proposes" (any phase NOTICES; only CONDENSE proposes), "approval flag" used loosely.
- **Phase-count and OPEVC-acronym (tunable surfaces)** — the layer-1 (phasic substrate) customization surface: the number of phases and the cycle's name are data, not hardwiring — a custom seed can add a phase (edit the FORWARD_MAP string + add the matching `phase_<name>` guard) or rename the acronym. The floor underneath: each phase publishes its restrictions ahead and the guard enforces them. _Avoid:_ "OPEVC is fixed" (the five phases are this prototype's answer, not law), treating phase count as code-deep (it is one map-string edit + a guard script).
- **Five customization layers (the gradient)** — the canonically-ordered altitude ladder (deepest/hardest → shallowest/softest): 1 phasic substrate · 2 hard controls · 3 soft voices · 4 per-job YAML (modifies existing voices only) · 5 per-job plugins (aspirational, Stage 4). Answers "at what layer does this customization belong?" — pick the shallowest layer that solves the problem; friction rises going down. A per-phase workflow *shape* is a learnable tuning surface, NOT a sixth layer. _Avoid:_ reordering the layers (the numbering is canonical), "config options" (flattens five distinct altitudes), "workflow shape is the sixth layer."

**Maturation model (Q-cluster — how a seed + its operator grow)**
- **Brain Maturation Model** — the canonical two-pole model of how a seed's cognition changes shape over time: young = large brain, few hooks, probabilistic guidance; mature = lean brain, many hooks, deterministic enforcement. Capability MIGRATES from brain to hooks (flow: `Experience → notice pattern → add to brain → pattern proves reliable → migrate to hook → remove from brain`). _Avoid:_ "the agent gets smarter" (it gets LEANER + more deterministic), "young = worse / mature = better," "everything eventually becomes a hook" (judgment controls stay soft), reordering the flow, "self-improving brain" (the operator drives migration).
- **Control types (hard, soft, structural)** — the three enforcement forms every plugin MIXES: hard (hooks that block, exit 2 — cannot be bypassed), soft (voice injections that guide — agent chooses), structural (design choices that make bypass impossible — the compartment shape itself). _Avoid:_ "hard > soft" (different jobs), "structural = hard" (structural makes the wrong move unrepresentable; hard catches it at the gate), reordering the three, "soft controls are weak."
- **Over-engineering veto (Lock 13)** — the design discipline gating the soft→hard flow: no control hardens before measurement proves the soft form is failing (voice-first precedence). _Avoid:_ "build the hook first to be safe" (inverts the veto — soft MUST be measured failing first), "Lock 13 blocks tools" (it is a design discipline, not a runtime gate).
- **Graduated enforcement** — the hook-design principle of escalating consequences progressively instead of one binary on/off switch; the canonical instance is brain_guard's context-tier ladder (positions tunable, the graduated shape invariant). _Avoid:_ "binary block," "graduated enforcement = the context tiers" (brain_guard is the instance; the principle is reusable), pinning tier percentages as fixed.
- **The cost ladder** — the four-tier per-control escalation **Voice → Hook → Plugin → Template**, each tier costlier, each entered only when evidence justifies the climb (the per-control mechanism that drives the Brain Maturation Model). _Avoid:_ "mandatory order" (a default entry sequence, not a mandate — enter at any tier on prior evidence), "automatic promotion" (operator-driven + evidence-gated), "Voice → Hook → Plugin → Template = the Brain Maturation Model" (the ladder is the per-control mechanism; the model is the whole-brain flow it drives).
- **The maturation arc** — the OPERATOR's three-stage growth arc **Apprentice → Journeyman → Architect** (friction, not enforcement — no mechanism checks or advances it). _Avoid:_ "Apprentice/Journeyman/Architect as seed states" (these stages describe the OPERATOR; the seed's parallel poles are young/mature per the Brain Maturation Model), "Architect" confused with the Layer-2 architect role (this is the END-USER's mature stage, not the prototype's Layer-2 session), fixed timelines (week 1 / weeks 4–12 / month 3+ are heuristics).

**CONDENSE waterfall & memory**
- **CONDENSE 7-step waterfall** — the strict ordered sequence CONDENSE runs to metabolise a cycle's working memory into durable form (it works the *memory layer* — CLAUDE.md footers + durable memory forms anywhere, never project deliverable files). The seven numbered steps are an operation CATALOG, not the execution order; the execution order is three gated phases: **ADDRESS** (marker-consumption steps 3–6) → **ARCHIVE** (step 7, mid-sequence) → **DEFLATE** (steps 1–2 + the footer deletion). The macro-order is hard-gated (removal-lock + archive-verify); within a phase the step order + destination are content-determined. Lock-forward only. _Avoid:_ "cleanup phase," "compression pass," "priority ladder," "steps are numbered in execution order," "CONDENSE edits the project."
- **The five CONDENSE markers** — the soft, EXTENSIBLE named-note system carrying a cycle's meta-cognition forward to CONDENSE: `[PENDING-JOB]{slug}` (→ job creation), `[VOICE-UPDATE]{voice_id | why | direction}` (→ voice.xml edit, existing ids only), `[AGENT-UPDATE]{agent_name | why | direction}` (→ subagent-def edit), `[KNOWLEDGE]{topic-slug}` (→ knowledge/ extraction), and `[DURABLE]`/`[EPHEMERAL]` (bare-tag routing bias). A *note* is the genus (any meta-cognition a phase jots into its footer); a *marker* is a NAMED note. The set is open (a phase may define custom sub-markers). _Avoid:_ "the four markers" (five core classes), "marker" for an unnamed free-form note, calling the four `---Xx---` anchors "markers."
- **`[DRAFT-TERM]` marker (job-custom marker class)** — the first canonical job-custom note class, the exemplar of the open marker set (kept SEPARATE from the five generics, which fire in every job): a vocabulary-touching job (the ref-tag review class) meets a term the shared-language files don't define and any phase drops `[DRAFT-TERM]{term-name | where met + what it seems to mean | suggested home}` into its footer. CONDENSE processes it via **main-session Edit — NOT a dedicated consuming subagent** (`condense.sh` excludes `draft_term` from the consumption-direction gate): the seed deposits a `[draft]` stub in the suggested context file (never `[consolidated]` — the flip is the user's), then rewrites the note to its terminal state `[RESOLVED:DRAFT-TERM]{breadcrumb}`. _Avoid:_ "a sixth generic marker," "a dedicated consuming subagent" (it is main-session-Edit-routed), "the seed consolidates terms."
- **Marked-note lifecycle** — the full life of a marked note from creation to terminal state. Two terminal states: `[RESOLVED:NOTETYPE]{breadcrumb}` (done — a verifiable work-reference) or `[CARRIED:NOTETYPE:n]{content}{why}` (deferred; `n` rises each surviving cycle). Governed by the 3 gated phases — ADDRESS (dispatch a shape-guarded subagent that quotes the live note verbatim, ONE note per dispatch; resolve or hand back) → ARCHIVE (footer snapshot) → DEFLATE (deletion). Hard controls fix ORDERING; soft controls shape QUALITY. _Avoid:_ "engagement = resolution" (a run is necessary but not sufficient — the note must reach a terminal state), "delete-then-archive," "guarantee every note is processed."
- **Four-footer protocol** — the partition every CLAUDE.md carries (brain AND project trees): a durable body on top, then four phase footers below, each opened by an **anchor** line — `---Ob---` / `---Pl---` / `---Ex---` / `---Ve---`. Footers are scratch and INFLATE as phases work; CONDENSE DEFLATES them (durable findings absorb up, the rest route sideways or leave with the session archive). The anchors are permanent and self-healing — only their content moves. _Avoid:_ calling the four anchor lines "markers" (they are anchors), "scratchpad" for the whole file (only the footers are scratch), "delete/clear the anchors."
- **Session archive (step 7)** — the ARCHIVE phase of the waterfall: a per-cycle FULL-footer snapshot written by the session-log command into the run-aware job dir `run-<r>/session-log-<c>.md`, executed mid-sequence (after marker-consumption, before deflation). The preservation step that lets the seed safely deflate — large by design. NOT where durable knowledge is found (that goes to body + `knowledge/`); reached only by breadcrumb. _Avoid:_ teaching `.claude/knowledge/session/` as the current home (legacy path), "short = healthy," "last-resort dustbin," "archive = a durable knowledge home."
- **Breadcrumb** — a soft trace-link CONDENSE leaves in the CLAUDE.md layer as it processes footer content: a short pointer (useful wording + a git ref / file path, ≤50 words, 1–2 sentences) naming WHERE a note's content or work landed. Existence is hard-gated (the `[RESOLVED:]` terminal state); prose stays voice-coached. A forward-reference breadcrumb is KEPT (points at surviving work); a relic-breadcrumb (dead pointer to removed content) is REMOVED. _Avoid:_ "breadcrumb = the deflation" (it is the trace MEANS; the deflation gate is the size MEASURE), matching breadcrumb count to note count (they batch).

**Voice surface**
- **voice.xml** — the plugin's soft-memory organ where all context-injection items are defined; each message carries a voice id and can be overwritten/modified by the Stage-3 yaml plan files. Six element types split soft vs hard: `<coaching>`/`<info>`/`<warning>` (soft — nudge via `additionalContext`), `<entry>` (priming), `<block>`/`<error>` (hard — `exit 2` refusal). The cheapest learning surface — CONDENSE tunes it via `[VOICE-UPDATE]` without a `[PLUGIN-LOCK]` ceremony. _Avoid:_ "voice file," "config" (tunable numbers live in `config.conf`), "prompt," treating `<coaching>` and `<block>` as the same thing.
- **Dual voice.xml** — every plugin carries TWO voice.xml files, split by FIRE-SITE (where the voice fires), not topic: `hooks/voice.xml` (agent-facing — injected into the LLM context) and `scripts/voice.xml` (operator-facing — printed to the human's terminal, never injected). The orphan audit must grep BOTH. 10 of the 11 active plugins carry both; `question_discipline` is hooks-only. _Avoid:_ "the voice file" (singular), "split by topic," "hooks = code / scripts = config."
- **Orphan voice** — a voice `<id>` defined in voice.xml but fired by NO hook or script: silent dead code and a trap for Stage-3 yaml authors who target it. The orphan invariant: every defined id must have a callsite, or be removed. Two sources: (a) an id in voice.xml with no callsite, (b) a `<coaching>` voice not appended to its tracker's `COACHING_IDS` rotation array. _Avoid:_ "unused voice" (the danger is the silent no-op), "dead voice" (acceptable informally; "orphan" is the project term tied to the audit tooling).

**Cross-system bridge**
- **ref-tag** — the inline three-field construct `*[ref: slug | source-pointer | content-summary]*` linking each teaching claim in the blog to the implementation that realizes it (the blog is the educational surface; `.claude/` is the ground truth; a ref-tag asserts both describe the SAME design). Content policy: STABLE + VALUABLE only — file-level and section/function pointers, **never line numbers** (not worth maintaining); content-summary ≤120 words. Coverage ≥80% is a SOFT editorial target, never a hard gate. A drifted tag is reduced by the ref-tag review job to stable useful content, never left stale. _Avoid:_ "reference tag," line numbers in pointers.
...
- The ALWAYS-ON plugins each own their own concern and their own data files. Their relationship to the CLAUDE.md layer **varies plugin by plugin**: `plugin_integrity` polices the four phase anchors from removal but explicitly EXEMPTS CLAUDE.md content from lock checks (per `plugin-guard.sh` — the "DOCUMENTATION + VOICE + AGENTS ALWAYS FREE" arm); the other four (`brain_guard`, `job_core`, `interaction_summary`, `question_discipline`) are orthogonal to CLAUDE.md, owning concerns (context budget, job lifecycle, interaction summarization, question discipline) entirely inside their own data files.
