...
**Why per-plugin scoping.** The safe-lock cycle inside `plugin_integrity` requires that only one plugin be unlocked at a time. A subagent that ranged across multiple plugins would need to coordinate locks across every directory it touches, defeating the unlock discipline. Per-plugin scoping makes the subagent's surface match the lock boundary exactly. *[ref: single-unlocked-plugin-invariant | .claude/plugins/plugin_integrity/hooks/lock-manager.sh (PreToolUse "block if already unlocked" guard; write_unlock_state()) | The PreToolUse guard reads `.unlocked_plugin` from data.json and blocks further unlock attempts if a plugin is already unlocked; `write_unlock_state()` sets that single field at lock acquisition. The invariant is enforced at the lock-acquisition step: only one plugin's `.unlocked_plugin` field can be non-empty at a time. Cross-plugin subagents would need to lock multiple plugins simultaneously, violating this invariant. Per-plugin scoping aligns the subagent's edit surface with the single-unlock contract.]*

**The 80/20 dispatch budget.** The architecture insists on the majority of cognitive work happening in subagents (the prototype targets roughly 80/20) and the remainder in the main session. Inside EXECUTE phase, this is mechanized: every phase entry seeds a starting balance of five direct actions, so the main session can begin building right away; every non-`.claude/` file edit draws against that balance, and every `execute-*` subagent dispatch replenishes it by three (configurable). The starting balance is the teaching device — the main session spends it down and learns that dispatch is how the budget refills. When the balance hits zero, the EXECUTE guard hard-blocks further edits and the only way forward is to launch another `execute-*` subagent. The 80/20 is not a guideline; it is a budget the main session must keep earning through dispatch. *[ref: execute-direct-action-budget-3-per-dispatch | .claude/plugins/phase_execute/scripts/execute.sh (grant-direct-action-budget / consume-direct-action-budget / direct-action-budget commands; BUDGET_STARTING seeding) + .claude/plugins/phase_execute/hooks/execute-guard.sh (has_direct_action_budget(); direct-action-budget-blocked block voice) + .claude/plugins/phase_execute/scripts/voice.xml `id="execute-tracker-coaching"` | Every phase entry seeds the budget with a starting balance of 5 direct actions (BUDGET_STARTING default 5; the meter is live from the first action, not dormant); each non-`.claude/` edit consumes a unit and each execute-* subagent dispatch grants +3. execute-tracker.sh documents the grant contract under "Budget Grant Model": launched execute-* subagents earn +3 direct-action budget grants. The scripts/voice.xml coaching element `execute-tracker-coaching`: "Each execute-* subagent launch grants +3 direct-action budget for file edits. Budget consumed -1 per non-claude/ edit." execute-guard.sh emits the BLOCK voice when the balance is exhausted: "Direct action budget exhausted. Launch an execute-* subagent to earn +3 budget."]*
