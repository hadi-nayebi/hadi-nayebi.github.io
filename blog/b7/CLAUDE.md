...
**EXECUTE re-entry (session 4, 2026-07-17) — SAME mis-route repeated; corrected mechanism established.** I again routed backward verify→execute to "regen" and again hit the execute-guard script-block ("run scripts in verify phase") — confirming session-3's finding a THIRD time. There is ZERO execute work (B-STUB `.md` caveats + `.gitignore` already in `9e5e6cb`; B-LINE empty; b8 clean). Returning to VERIFY empty via `execute-commit.sh --force`. **Corrected full mechanism (session-3's note was HALF-right):** the regen runs in verify ✓, BUT verify-commit `--force` stages ONLY CLAUDE.md + plan files (`_stage_verify_targets_in_repo`) and its post-commit `require_clean_all_repos` (exit 2) REFUSES the advance while the regenerated `.html` is uncommitted — so verify-commit `--force` CANNOT ship the `.html`. The `.html` deliverable ships via the **backward-to-execute `--commit` sweep** (`verify-commit.sh --backward execute --commit` → `git add -u` folds the regen'd `.html` into the commit), OR an execute REGULAR commit with `.html` file-args (the b5 precedent). Net verify→condense path once back in verify: **regen 07_5+07_6 → `verify-commit --backward execute --commit` (ships `.html`, free) → `execute-commit --force` back to verify → re-check G2 idempotence → `verify-commit --force` to condense.**

[KNOWLEDGE]{ref-tag-html-regen-two-step-ship — CORRECTS the earlier mirror-gap note. In a website ref-tag job the `.html` regen+ship is a TWO-STEP that spans verify AND execute, because the two phase-guards split the capability: (1) the approved `generate_blog_html.py` regen runs ONLY in VERIFY (verify-guard honors `allowed_commands`; execute-guard does NOT — it blocks ALL scripts). (2) the regenerated `.html` is a project deliverable that verify-commit `--force` will NOT stage (it stages only CLAUDE.md+plans) and whose uncommitted presence trips verify's clean-git invariant — so the `.html` must be COMMITTED via a backward-to-execute path (`--backward execute --commit` `git add -u` sweep, or an execute regular commit with `.html` file-args). Therefore the minimal clean loop is: VERIFY regen → backward-to-execute commits the `.html` (FREE) → forward `--force` back to verify → verify `--force` to condense. A ref-tag job with `.html` edits ALWAYS pays one verify→execute→verify round-trip for the `.html` ship; that is structural, not a mistake. The mistake is routing backward-to-execute to "regen" (execute can't) instead of AFTER regen to "commit". Captured 2026-07-17 session 4.}
