...
**Two protected contexts for plugin-substrate edits.** Editing an existing plugin *or birthing a new one* requires one of two protected contexts: (A) `[GMODE]` with a body long enough to surface the cognitive work (currently a 100-word floor in the prototype, configurable) — the operator's deliberate maintenance lane; or (B) a focused job whose top-level `plugin_lock_approval` flag is set — established by the user confirming an agent-proposed `[JOB-APPROVE-CREATION]` question. New-plugin birth does not bypass the gate. Creating a plugin writes the agent's enforcement substrate too, so the same approval rule applies whether the target directory already exists or not. *[ref: plugin-lock-gmode-or-approved-job-gate | .claude/plugins/plugin_integrity/hooks/lock-manager.sh:202-238 | L202 names the gate: "Gmode-OR-plugin_lock_approval gate (PLUGIN-LOCK admission — D71 + A14 + 2026-05-19)." L203-208 state that `[PLUGIN-LOCK]` for BOTH existing plugin edits AND new plugin birth admits on either `current_phase == "gmode"` or the focused job's `plugin_lock_approval == true`. L208-209 record the correction: the gate also applies to plugin BIRTH; prior behavior bypassed birth. L225-229 read the top-level `plugin_lock_approval` arm from `job_core/data.json`. L232-238 block when both arms fail and distinguish the block text as "existing plugin edit" vs "new plugin birth." This is the current prototype behavior and the B5 ground-truth rule.]*
...
Two-tier lock — PLUGIN-LOCK opens the cell; TEST-LOCK opens a specific shell test file inside the cell. The agent never gets a blanket "all shell tests editable while the plugin is unlocked" pass. Each shell test file the change touches earns its own ceremony. *[ref: test-lock-handler-gated-on-plugin-unlock | .claude/plugins/plugin_integrity/hooks/lock-manager.sh:393-411 | The TEST-LOCK handler enforces two-tier by code structure. L394-399 require a plugin already unlocked: `current_plugin=$(jq -r '.unlocked_plugin // empty' "$DATA_FILE")`; if empty, L398 emits the block "[plugin_integrity] BLOCKED: No plugin unlocked. Unlock a plugin first before unlocking tests." and L399 exits 2. TEST-LOCK has its own word-floor independent of PLUGIN-LOCK's — L407-408 enforce `TEST_LOCK_WORD_MIN:-100` with its own block voice "test-lock-short-description" (L409-410). PLUGIN-LOCK sets `unlocked_plugin`; TEST-LOCK sets `unlocked_test`; the two-field schema (`lock-manager.sh:127`) decouples the locks by design.]*
