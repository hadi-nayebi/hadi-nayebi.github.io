...
**S-16 RULING — loop-break:** STOP hand-editing `.html` (Rule-32 anti-pattern; no-Grep-in-execute → whole-file Read → the 14-compaction loop). **REGENERATE all 5 fixed essays' `.html` from `.md` in VERIFY** (Bash-native; PLAN method ~L480), overwriting prior hand-edits, then grep-at-zero per family. VERIFY also owes: 05_9 own-eyes 1/3/5; F5-candidates (05_7 L127/L155, 05_1 L138/L142/L184/L186) grep-verify. F4 table→CONDENSE. If VERIFY blocks the generator write → name the exact block + escalate the substrate gap; never resume hand-editing.
