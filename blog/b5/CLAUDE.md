...
**S-12/13 applied:** 05_3 F1 (6 accrual tags→%-of-window) in `.md`. **05_1 DONE .md+.html:** F5 `cognition-as-memory-multi-form` stale quotes reduced (sole→durable/monotonic; 5→one-per-fact, 6 facts); F3 `single-concern-principle-minimize-rule` line-nums stripped. **KEY:** .html edits belong IN EXECUTE (VERIFY read-only, no regen) → `.html` OWED for md-fixed 05_2/05_3/05_6; 05_1 F5 brain_guard/settings candidates→VERIFY grep.
