...
**S-16/S-18 RULING — loop-break + VERIFY resume:** The 14-loop was HAND-MIRRORING `.html` in execute, NEVER the actual generator. All `.md` fixes applied (S-11→S-15); HEAD `f0fad45` touched no `.html` → regen NOT landed. **F1 grep-at-zero FAILS on 05_3.html:** L193 `accrual-boundaries-tunable-config` + L203 `tier-count-customizable` still carry `ACCRUAL_SOFT`/`ACCRUAL_READ_BLOCK`. ACTION (Bash-native, the designed mechanic): regen the 5 fixed essays via `generate_blog_html.py` in VERIFY → grep-at-zero per family → 05_9/05_7 own-eyes 1/3/5 → F5-candidates (05_7 L127/L155 · 05_1 L138/L142/L184/L186) → F4 table→CONDENSE → done-[WAITING] to architect. If the guard blocks the generator `.html` write → record the exact block + escalate the substrate gap; never hand-edit.
