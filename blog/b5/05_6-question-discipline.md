...
The same discipline generalizes to nearly every prefix; each body shape names what the question is about, which forces the seed to decide what it's about to do BEFORE handing the choice to the user. The coverage gap has largely closed — every registered prefix except `[COMMAND-APPROVE]` (deliberately shapeless) now carries a shape rich enough that filling it is the thinking — and the registry stops being a membership filter and becomes a slot-cognition filter that turns each ask into a deliberate, structured move.
