...
There is one more move, and it leans on the whole point of the seed agent: the mind lives on disk. After a compaction, the conversation has shrunk, but the focused job, the current phase, and the working memory all survived — they were never in the chat to begin with. So the moment the fresh session is born, the digital cortex re-grounds itself: it resets its context-fill tracking, reads the focused job and the live phase straight off disk, and re-presents the same phase-entry coaching the agent heard when it first entered that phase — the scope it is allowed to touch, the forecast it owes. The keystroke follow-up still kicks off the first turn; a reflex that fires on session birth can hand the agent its bearings, but it cannot make the agent start talking. That division is deliberate: the re-grounding is automatic and certain, so even if the follow-up keystroke mis-fires, the fresh session already carries its own orientation. *[ref: compact-wake-reflex | .claude/plugins/brain_guard/hooks/compact-wake.sh + settings.local.json SessionStart compact matcher | compact-wake.sh fires on SessionStart(source=compact). Four deterministic actions: (1) set-last-tier 0 (soft-warning high-water restarts — context shrank); (2) orientation digest = focused job id/name/objective-first-sentence + current phase + cycle, read LIVE from job_core/phasic state; (3) re-injects the current phase's `entry-set-multiplier-pre-set` entry voice (multiplier contract + allowed surface + phase duties; idle/gmode → orientation only); (4) idempotent pin_pane re-derive (does NOT touch pin_wid). Header comment: "A SessionStart hook INJECTS context but cannot START A TURN — so the self-compact FOLLOWUP paste remains the turn-trigger. This reflex makes the orientation DETERMINISTIC: even if the paste mis-fires... already carrying the digest + phase voice." Fires only on source=compact (clear/startup/resume excluded by design); every error path exits 0 (must never brick startup).]*
