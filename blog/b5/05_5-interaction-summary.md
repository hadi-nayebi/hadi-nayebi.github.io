...
What you would **not** do is let the mega-prompt grow without compression. Without the gate, a job that crosses ten cycles becomes unreadable; the next session loses the narrative thread; the agent starts drifting from prior decisions. *[ref: interaction-summary-compression-gate | .claude/plugins/interaction_summary/CLAUDE.md Objective | the summary gate compresses the growing mega-prompt so a long job stays legible across sessions]* The gate is the floor; the format above it is yours.
