...
The **phasic plugins** generate the bus's content. The choreography of how each phase writes its footer and how CONDENSE absorbs them is the subject of the [Essay 6 series](../b6/06_1-phasic-foundation.html). *[ref: phasic-plugins-generate-bus-content | .claude/plugins/phase_condense/scripts/condense-commit.sh (anchor-grep + word-count in deflation gate) | CONDENSE deflation gate greps `^---(Ob|Pl|Ex|Ve)---$` first anchor in every altered CLAUDE.md, counts words AFTER (all four footers combined), computes `words_absorbed = total_baseline - current_total`. All four phasic plugins write through these markers.]*
