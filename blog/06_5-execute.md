...
EXECUTE writes two things: the code, and *execution notes* in the working CLAUDE.md. The notes are short — what surprised the agent, what decisions the agent made when the plan left a judgment call open, what the next phase should know. The notes are what turn a sequence of commits into a coherent narrative. They will be one of the things CONDENSE absorbs. *[ref: execute-tools-table-claude-md-as-notes | phase_execute/CLAUDE.md Tools section | EXECUTE's tool table labels project-file edits (5 pts) and CLAUDE.md edits (10 pts, "Execution notes — resets synthesis counter") as the phase's two write activities, with notes earning the highest reward among edits.]*
