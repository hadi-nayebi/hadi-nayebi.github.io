...
A plugin form of a job extends each phase's guard with job-specific rules. The plugin lives at `.claude/plugins/<job_name>/` like any other plugin — with hooks, scripts, tests, voices, agents, knowledge dir — but its hooks attach to the standard OPEVC events with logic that recognizes when the focused job matches this plugin's job-type and applies extra constraints. Outside that job-type, the plugin's hooks are pass-through. *[ref: plugin-template-as-job-form-skeleton | .claude/plugins/plugin_integrity/template/ + .claude/plugins/phasic_system/hooks/phase-gate.sh + settings.local.json | The universal plugin skeleton lives at `plugin_integrity/template/` (CLAUDE.md + docs/ + _historian.md, stamped on birth via lock-manager.sh L313-341). Any job-form plugin inherits the same organ shape (`hooks/`, `scripts/`, `tests/`, `template/`, `agents/`, `docs/`) — verifiable across the 13 plugins (config.conf=10, template/=6, tests/=13). Hooks attach to OPEVC events via `settings.local.json` registration; the phasic_system orchestrator dispatches to whichever plugins registered hooks for the current event. Job-type matching reads `data.json` focused-job fields and short-circuits when the focused job is not of this plugin's type.]*
