...
A patent attorney whose seed has authored a `prior-art-search` plugin will, eventually, need to fix a bug in that plugin's own search-result-deduplication logic — and the fix will require the attorney's seed to issue the lock for the plugin, run that plugin's own tests, pass them, and let the plugin lock itself before the change is committed. The same recursive ceremony described below for the prototype's `plugin_integrity` applies to every plugin the operator's seed has authored, in every operator's domain. The recursion is not a special case of the prototype; it is the architecture. *[ref: recursive-ceremony-spans-every-plugin | root CLAUDE.md "Identity" section | Fact 1 names the architecture extensibly: "You are extendable. Your plugin layer is a starting kit, not a finished form. New plugins can be added; existing plugins can be customized..." Fact 2 names the universal PLUGIN-LOCK scope: "Editing your own plugin layer alters the substrate that enforces every other plugin's discipline. That kind of work can only happen inside one of two protected contexts: gmode... OR a user-approved job." Combined: PLUGIN-LOCK is plugin-name-agnostic; every operator-authored plugin enters the same recursive lock ceremony.]*
...
Every part of the loop has a guard. Every part has a self-test. Every part has a coaching voice and a structured block. The architecture does not trust the operator to remember the rules. It does not trust the agent to remember the rules. It encodes the rules into the parts that touch the work, and lets the parts enforce themselves on each other. *[ref: every-plugin-kit-hooks-scripts-tests | .claude/plugins/CLAUDE.md "What This Directory Is" section | The opening section's verbatim sentence names the kit each plugin ships: "Each subdirectory is a **plugin** — a self-contained unit that owns one concern and enforces it through hooks, scripts, and tests." The body's refrain (guard + self-test + coaching voice + structured block) maps to this kit: hooks/ contains both the structured block (exit-2 guard) and the coaching voice (voice.xml lives inside hooks/ or scripts/); tests/ contains the self-test; scripts/ contains the worker logic that hooks call.]*

## Why This Matters for Reliability

The reason this matters is the reason agent reliability has been hard. A reliable agent is one whose behavior you can predict from cycle to cycle, not a clever one whose tricks impress in a single session. Predictability across time requires that the agent's *substrate* — its rules, its hooks, its phase boundaries — outlive any single session and any single LLM context. The seed agent is built so that the substrate does outlive those things. The chat dies. The model rolls forward. The operator forgets. The brain remembers, because the brain is on the filesystem, protected by guards that the filesystem itself enforces. *[ref: brain-on-filesystem-survives-session | root CLAUDE.md "Identity" section | The Identity section's opening sentence names the substrate verbatim: "You are a **seed agent** — a customizable cognitive system that lives on disk, organized through plugins, and grows with the work you do. This file is your brain's index; the rest of `.claude/` is the brain." "Lives on disk" places the substrate on the filesystem. "Organized through plugins" places the guards on the same filesystem, owned by the plugin layer that polices its own edits. The filesystem persists across sessions; the chat does not.]*
...
That is the seed agent. The limit on the guarantee is the test coverage: a plugin whose own tests miss the breaking case will let a broken edit land. The architecture trusts the operator's seed to author and maintain tests; the rollback substrate works only as far as the tests reach. *[ref: rollback-bounded-by-test-coverage | .claude/plugins/plugin_integrity/scripts/safe-lock.sh | L3 names the one-line contract: "Locks a plugin: run tests → pass: commit + lock / fail: revert + lock." The revert path fires only when a test exits non-zero (L258 trigger_reason enum: `tests-failed|tests-timed-out`). If a plugin's tests don't exercise the breaking case, no test fails — the script commits the edit. The rollback substrate reaches only as far as the tests themselves do.]*
