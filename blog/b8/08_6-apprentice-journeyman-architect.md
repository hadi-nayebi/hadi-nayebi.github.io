...
The lessons are accumulating in three places: the knowledge directory under `.claude/knowledge/<topic>/`, the memory layer in your home directory, and each plugin's `docs/evolution.md`. The brain is bigger at the end of week one than it was at the start — apprenticeship grows by accretion. *[ref: three-accumulation-sites-knowledge-memory-evolution | .claude/CLAUDE.md "Components" section + root CLAUDE.md "Sub-Operations of CONDENSE" section | The "Components" section names `knowledge/` as "Long-term condensed knowledge organized by topic. Written during CONDENSE phases. The sole reference for building and decision-making" and lists each plugin under `plugins/` carrying its own `docs/evolution.md`. The "Sub-Operations of CONDENSE" section names step 6 — knowledge files routed by topic — as the consumption of `[KNOWLEDGE] topic-slug` markers. These two routing mechanisms plus memory at the operator level are the three accumulation sites the apprentice phase fills.]*
