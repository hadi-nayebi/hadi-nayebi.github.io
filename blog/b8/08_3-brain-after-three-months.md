...
**The session archive is the prototype's fallback tier.** In `phase_condense`'s design it is the destination of last resort — content that did not route to any earlier waterfall step. When it grows large relative to the topic-organized silos, that is an honest signal that earlier routing under-fired; mature seeds compress it as their condense subagents learn to route more aggressively into topic-organized files instead. *[ref: session-archive-fallback-tier-size | .claude/plugins/phase_condense/docs/principles.md "session as last resort" principle | The phase_condense design places `session/` at step 7 (last-resort fallback), NOT as a peer of the topic dirs. Size signals under-firing of earlier steps (step 1 absorb-footer, step 2 cross-file migration, step 6 knowledge routing). The directory's word count is volatile — test resets condense it — so it is not pinned here: live `find .claude/knowledge/session -name "*.md" \| xargs wc -w` is authoritative.]*
