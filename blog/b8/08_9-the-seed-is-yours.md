...
The Hadosh Academy seed agent is open source. MIT licensed. Free to use, free to fork, free to extend. There is no SaaS layer between you and your seed. No server holds your knowledge directory. No company controls your brain. *[ref: seed-agent-mit-public-repo | seed_agent/LICENSE | The repository carries the literal MIT License — verbatim "MIT License" header, "Copyright (c) 2026 Seed Agent Contributors", "Permission is hereby granted, free of charge". The MIT terms permit use, fork, and extension without royalty or contractual gate, matching the body's "free to use, free to fork, free to extend." Root CLAUDE.md "Workspace Structure" section names the same directory as the "PUBLIC SEED AGENT" (`seed_agent/  # PUBLIC SEED AGENT (own git, public repo)`). No SaaS layer, no server, no central control — the license + the public-repo placement are the structural evidence of the body's claim.]*
