...
When you install a seed on your laptop, it becomes yours. The Claude Seed described across this series is one reference implementation: a worked answer to one operator's problems, built deeply enough that its patterns can be studied. What should travel forward is not every file, threshold, phase name, or enforcement trick. What should travel is the understanding of what those mechanisms were trying to accomplish. Your own jobs, voices, hooks, memory forms, phases, and plugin boundaries should emerge from the cognition and work you actually want to externalize. *[ref: reference-implementation-not-closed-form | root CLAUDE.md Identity Fact 1 + .claude/context/identity.md Phase-count and OPEVC-acronym customization surface | Fact 1 says the plugin layer is a starting kit, not a finished form, and the consolidated identity glossary says the specific compartments are this prototype's answer rather than universal law. Phase count, phase names, memory forms, and plugin shapes are customization surfaces.]*

## Patterns, Not Ports

That distinction changes how I build with people. I rarely want a new user's harness to begin as a code copy of an old one. I would rather point the new CLI agent at prior prototypes as a pattern library, ask what worked and why, and then build the new harness again with the user from a clean starting point. A stop gate may survive as a concept while its state model changes. A job may remain the unit of work while its lifecycle changes. A phase may remain a constrained mode of cognition while its name, tools, transitions, and enforcement change.

The repeated rebuilding is not waste. It is how we learn which abstractions are stable. If a concept remains useful after the code, model, framework, operator, and domain all change, it has earned more confidence than a mechanism that only works inside one prototype. And because the user is involved while those decisions are made, they learn their own harness as it comes into existence. They can choose terminology that makes sense to them, know which behaviors already exist, and later collaborate with their CLI agent to extend the system without depending on the person who built the first version.

Two operators six months in may share recognizable ideas — jobs, behavioral plugins, cognitive phases, local memory, authority boundaries — without sharing the same architecture in detail. Their terms can differ. Their plugin anatomy can differ. Their enforcement can differ. The common layer is the useful abstraction; the lived cortex belongs to the person. That is the design. *[ref: lived-seeds-diverge-by-design | root CLAUDE.md Identity Fact 1 | The identity layer explicitly says the specific compartments are this prototype's answer to one user's work and that a cultivated seed may invent memory forms and plugin shapes that fit its own work. The invariant is extensibility and safe customization, not identical implementation.]*
...
[Essay 5.1](../b5/05_1-the-two-layer-foundation.html) gave you one working substrate. [Essay 6.1](../b6/06_1-phasic-foundation.html) gave you one concrete cognitive-cycle design. [Essay 7.1](../b7/07_1-plugin-kit-foundation.html) gave you a vocabulary for reasoning about a plugin as a bounded behavioral system. This essay closes the loop: the point is not to memorize this prototype. The point is to become capable of recognizing, naming, and composing the cognitive structures your own work needs. The harness is yours; you are the architect. *[ref: stage-arc-grounded-in-job-stages | root CLAUDE.md "Job Stages" section | The Claude Seed's Job Stages remain one implemented maturation path: Stage 1 single-cycle collaborative, Stage 2 repeatable .md plan, Stage 3 repeatable .yaml plan, with an aspirational Stage 4 plugin form. They are evidence for how work can mature into structure, not a requirement that every future harness use the same taxonomy.]*
...
*The series ends here. The public Seed can become one starting point. Your digital cortex begins when you decide what should be yours.*