...
Three organs, one shared edit gate. CLAUDE.md is the brain surface; hooks are the reflexes; scripts are the verbs. Every plugin carries all three — and every edit to any of them passes through the same PLUGIN-LOCK ceremony. The next sub-essay opens the organ that almost every plugin doubles — `voice.xml`, once for the LLM and once for the operator.
