...
CONDENSE then closes the cycle by absorbing what it produced: the four footer-marker sections (OBSERVE, PLAN, EXECUTE, VERIFY footers from [Essay 5.7](b5/05_7-claude-md-hierarchy.html)) plus the cycle's inline markers (pending-job, voice-update, agent-update, knowledge — the inter-phase protocol from [Essay 7.2](07_2-skeleton-claudemd-hooks-scripts.html) and surrounding essays). The absorbed content lands in the knowledge layer alongside the operator's preferences, the files that mattered, and the discipline that emerged.
