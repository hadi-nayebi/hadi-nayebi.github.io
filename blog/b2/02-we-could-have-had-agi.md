...
Not the sci-fi kind. Not a god in a box. The practical, AGI-like kind this essay argues for — a system that learns a profession, keeps working for weeks, gets better from reviewed experience, and does not need someone watching it every thirty minutes. The kind you can run on your own computer without a research-scale budget.

Not a fixed artifact with a fixed outcome. A **complex system** — one where capabilities emerge from simple parts interacting and the engineered structure can adapt. The way a living organism emerges from cells, not from a single blueprint.
...
The hard part is not generating text. The hard part is staying on every step, holding scope, remembering constraints, and avoiding the same mistakes twice. None of that survives when earlier [tokens](https://en.wikipedia.org/wiki/Large_language_model#Tokenization "Small word-sized chunks that AI models use to process text") leave the active [context window](https://en.wikipedia.org/wiki/Large_language_model#Prompt "The limited amount of text an AI can 'see' at once") through compaction, pruning, or a new session. We solved the generation problem. What we never solved is the preservation problem — how to keep those tokens organized, accessible, and persisted into something durable past the next session. A bigger generator just produces more tokens to lose.
...
Imagine nature working only with [RNA](https://www.ncbi.nlm.nih.gov/books/NBK26876/) — a family of molecules that includes information carriers and catalytic ribozymes. Now imagine evolution trying to scale that one family into a giant monolith that stores all instructions, catalyzes all reactions, and regulates itself internally. In principle, maybe it could work.
...
Now think about another complex system: the brain. In [*A Thousand Brains*](https://www.frontiersin.org/journals/neural-circuits/articles/10.3389/fncir.2018.00121/full), Jeff Hawkins and colleagues describe cortical columns as association engines: they learn relationships among what is sensed, how the body moves, and what the brain expects to sense next. LLMs work through a different mechanism, but they reveal the same architectural power in linguistic and semantic space — learning associations across context and anticipating what belongs next. The comparison is not biological equivalence. It is the shared pattern: intelligence deepens when learned relationships meet, specialize, and coordinate. A single column can learn and predict. A single model can generate and generalize. But intelligence did not emerge from one enormous super-column. It emerged through interacting circuits, specialized regions, and coordinated subsystems.
...
And that replaceability is becoming practical. New kinds of language engines are emerging, including [diffusion-based language models](https://openreview.net/forum?id=j1tSLYKwg8). A well-designed agent can care less about which compatible engine powers it. When the next engine arrives, adapters may change while much of the structure persists.
...
But one primitive is not obvious at all, and it changes everything: resist stopping before the work is done. [Ralph Loop](https://github.com/anthropics/claude-plugins-official/tree/main/plugins/ralph-loop) repeats a prompt through a Stop hook until a completion signal, iteration limit, or cancellation. Repetition does not guarantee quality, but the loop shows how one hook can sustain work beyond a single attempted finish.
...
Define a persistent list of **jobs** on disk (not in the model) — a local file in a structured format like JSON. Mark jobs active or completed — or add whatever states and fields the work demands. Where the runtime supports it, install a hook that intercepts "Stop" while a job is active, with bounded retries, cancellation, and recovery.
...
Then you scale it. Every job carries an observation field — what happened, what was learned, what went wrong. Completion checks can require the agent to update it and route reviewed lessons before the job closes. The system cannot mark "done" until it has processed the evidence you require.

You now have a system that can refuse to close until required observations become durable. It must metabolize its stream of tokens into something reviewable. It must tidy its state.
...
Hooks are part of the agent's sensory layer — how it perceives selected actions and reacts to events. A hook records what you configure it to capture. That record can become raw material for learning: you review it, see what worked, and tighten the rules.
...
The important property is not what the seed contains on day one. It is that the seed can grow under its user's authority. As the agent works, reviewed patterns get codified, operations emerge from experience, and guardrails get added after failures. The structure accumulates — but the identity holds.
...
The other sits inside a filesystem brain. It logs what matters and digests reviewed lessons before closing each job. After a year, it may have accumulated case notes, a playbook of failure modes, specialized templates, refined guardrails, and an evolving organization of knowledge.
...
Earlier attempts at agent autonomy — AutoGPT, BabyAGI, and their descendants — used engineered loops, roles, and task flows. When a fixed plan did not fit reality, it could break. Their limits did not disprove engineering; they showed why state, recovery, verification, and user control matter.
