...
[Essay 7.2](07_2-skeleton-claudemd-hooks-scripts.html) opened the universal skeleton — CLAUDE.md, hooks, scripts — and named the PLUGIN-LOCK ceremony that gates every edit to them. This sub-essay opens the organ that almost every plugin doubles: `voice.xml`. Two files, same XML schema, two audiences. Getting them confused is the most common new-user error in plugin authoring.
