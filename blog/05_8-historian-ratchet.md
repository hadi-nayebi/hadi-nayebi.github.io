...
Call this **composed ceremony**: narrow parts, structured interfaces, emergent rituals — the same shape as the `/compact` five-section template, the same shape as the ratchet itself. Narrow constraints composing into behaviors larger than any single constraint. *[ref: composed-ceremony-narrow-parts | .claude/plugins/CLAUDE.md Control Types in Plugins section | Control Types in Plugins: "Hard controls — hooks that block (exit 2)" + "Soft controls — injections that guide via voice" + "Structural controls — design choices that make bypass impossible". Each plugin mixes all three; emergent behavior from narrow control forms.]*
