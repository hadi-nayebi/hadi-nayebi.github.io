...
Below is the current prototype's always-on layer. One paragraph each — what each owns, what would break without it, and the one mechanism inside it worth knowing about.

**`plugin_integrity`** — owns enforcement of safe edits across every plugin. The mechanism is a script called `safe-lock.sh`: when an editing session closes, it runs every `.sh` test file under the affected plugin's `tests/` directory, then either commits the change (tests pass) or reverts the working tree to a captured checkpoint SHA (tests fail). Each revert appends a structured entry to a 20-deep FIFO log inside the plugin's hidden `data.json` — timestamp, list of failed tests, list of files restored, pre-revert SHA, trigger reason — and emits an `auto-revert-fired` voice line to stderr so the operator cannot miss it. There is a richer ceremony around *unlocking* a plugin for editing in the first place; that mechanism is deconstructed in [Essay 7](07-the-plugin-kit.html). For this essay, what matters is the simpler always-on role: every plugin edit passes through a test gate, and a failed gate undoes the change. Without this, every plugin edit is a coin flip — the system might break, and the breakage might not surface until it misfires hours later.

**`brain_guard`** — owns the conversation context budget. A `PreToolUse` sensor (`context-sensor.sh`) reads the last transcript message's token usage on every call and watches for tier crossings: soft at 200k, hard at 250k, critical at 300k. The model itself can hold much more (the seed runs on Opus 4.7's million-token window), but `brain_guard` triggers a structured compaction long before the ceiling, while there is still enough headroom to reason about what to preserve. At each tier, the plugin injects a coaching voice and prompts the agent to draft a five-section compaction instruction. When the agent submits `/compact`, a script called `self-compact.sh` types the instruction into the active terminal — through `xdotool` and `xclip` under X11, or through `tmux set-buffer` and bracketed paste under tmux — and writes a `last_self_compact_at` timestamp into the plugin's hidden `data.json`. The hard-tier guard reads that timestamp on its next firing and grants a 300-second grace window: just enough time for the compacted transcript to register before the gate would otherwise re-block. Without `brain_guard`, the agent runs the conversation off a cliff and the next session starts from scratch with whatever happened to be in the chat at the moment of failure.
