...
Plugins evolve constantly. Every time the agent surprises the operator, a check might be added, a voice re-tuned, a script tightened. If editing a plugin were as casual as editing any file in the codebase, the brain would degrade as fast as it grew.

The seed agent makes plugin editing *expensive*.

Editing any plugin requires a three-part ceremony — `PLUGIN-LOCK`, `TEST-LOCK`, and the safe-lock cycle. Each part exists for a reason.

**`PLUGIN-LOCK`** is how editing begins. The agent issues a structured question to the user with the prefix `[PLUGIN-LOCK] <plugin_name>`. A `PostToolUse:AskUserQuestion` hook called `lock-manager.sh` watches for that prefix. When the user approves, `lock-manager.sh` runs `git rev-parse HEAD`, captures the resulting SHA as a `checkpoint_ref`, and writes both the plugin name (`unlocked_plugin`) and the checkpoint into `plugin_integrity`'s hidden `data.json`. From that moment on, a `PreToolUse:Edit/Write` guard called `plugin-guard.sh` allows edits to that plugin's directory and rejects edits to every other plugin's directory. Only one plugin is editable at a time. The rest are, for the moment, frozen.

The reason for this constraint is that plugin edits cascade. Touching two plugins in the same cognitive breath — fixing a hook in one, adjusting a script in another — almost always means the operator is missing the connection between them. The lock forces serialization. Edit one plugin completely, commit it, hand back the lock, and only then unlock another.

The `[PLUGIN-LOCK]` question is itself one of the prefixes the always-on `question_discipline` plugin enforces. The system would not let the agent ask the question without the right prefix. The system would not let the agent edit a plugin without the right answer. The two always-on enforcements compose into the lock ceremony.

**`TEST-LOCK`** is finer-grained. The plugin's test suite is, by default, frozen along with the plugin. To touch an individual `test-*.sh` file, the agent has to issue a separate `[TEST-LOCK]` question naming the specific test file. The reason is the same one that makes test-driven development hard: it is too easy to "fix" a test that is correctly failing. Test edits demand explicit, named permission, separate from the plugin edit itself.

**Safe-lock auto-revert** is the cycle that closes a lock. The agent has two ways to close it. The clean way is `lock-cmd.sh`: it runs the plugin's full test suite, and on success commits the change and clears `unlocked_plugin` in `data.json`. On failure it stops and leaves the working tree in place so the agent can fix the test before retrying. The defensive way is automatic: if `plugin-guard.sh` detects the agent trying to edit outside the unlocked plugin, `safe-lock.sh` runs the suite anyway, and on test failure rolls the working tree back to the captured `checkpoint_ref`, appending a structured entry to the 20-deep FIFO revert log inside `data.json`. Either way, the agent does not get to ship a plugin change that breaks the plugin's own self-test. There is no override.

This is the discipline that lets the brain mature. Every change is gated by a lock. Every commit is verified by tests. Every failure is recorded.

There is a fourth piece, the **historian ratchet** from Essay 5: the lock can refuse to open at all if `evolution.md` has fallen behind the plugin's git history. Together, the four pieces — lock, test-lock, safe-lock, ratchet — form a closed loop where the only way to evolve a plugin is to have already understood it.

This is why the previous essay's phasic plugins can be trusted to enforce their own rules. They were built under the same discipline. The architecture is recursive: plugins enforce work on the project; plugin discipline enforces work on the plugins themselves.
...
The second flavor is **structured blocks**. A block is also a string, but it is delivered as a hard refusal. When a guard fires — a write tool aimed at a path outside the phase's scope, an `AskUserQuestion` without a registered prefix — the plugin returns a non-zero exit code with the block string as the explanation. The agent's tool call fails. The block is not a suggestion; it is a denial.
