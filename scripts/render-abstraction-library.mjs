...
<main>
  ...
  <section class="container abstraction-cloud-section" aria-labelledby="term-map-title"><div class="term-category-heading"><div><span class="abstraction-eyebrow">Living vocabulary</span><h2 id="term-map-title">A map of the current terms</h2></div><p>Size suggests how centrally a term currently connects to the rest of the seed. It is navigation, not a claim of permanent importance.</p></div><div class="abstraction-cloud" role="navigation" aria-label="Canonical abstraction terms">${lib.terms.map(t=>`<a href="/agents/abstractions/terms/${t.slug}.html" data-weight="${t.weight}">${escapeHtml(t.name)}</a>`).join("")}</div></section>
...
<main>
  <article>
    <header class="term-hero"><div class="container"><div class="term-breadcrumb" role="navigation" aria-label="Breadcrumb"><a href="/agents.html">Agents</a><span>→</span><a href="/agents/abstractions/">Abstraction Library</a><span>→</span><span aria-current="page">${escapeHtml(term.name)}</span></div><div class="term-state-row"><span class="badge">${escapeHtml(term.status)}</span><span>${escapeHtml(category.name)}</span><span>${escapeHtml(term.id)}</span></div><h1>${escapeHtml(term.name)}</h1><p class="term-definition">${escapeHtml(term.definition)}</p></div></header>
